...
<div class="container">
	...
	?>
		...
		<?php 
			...
			$awb = $data['npd_hawb'];
			$tglawb = $data['npd_tgl_hawb'];


			$sql_catatan="SELECT * FROM npd_catatan WHERE awb='$awb' AND tgl='$tglawb'";
			$query_catatan=$DBcon->query("$sql_catatan");
			$datacathitung=$query_catatan->num_rows;

			if($datacathitung>0){
				$datacat=$query_catatan->fetch_array();
				$flagcat=1;
			}

		?>
			<tr>
				<th colspan="6">Riwayat dokumen HAWB : <?php echo $data['npd_hawb']; ?> tanggal : <?php echo $data['npd_tgl_hawb']; ?> PJT:<?php echo getpjt($data['npd_id_pjt'],$DBcon); ?> <?php if($datacathitung>0){ echo '<a target="_blank" href="catatan.php?&id='.$datacat['id'].'">&nbsp;&nbsp;[Lihat Catatan]</a>';  };?></th>
			</tr>
