	...
	<div class="container">
		<div class="row">
			<p>Data dokumen siap dilakukan penelitian kembali ex notifikasi, Urutan data berdasarkan waktu pengajuan dokumen dari yang paling lama</p>
			<p>Jumlah maksimal data untuk sekali proses sebanyak 100(seratus) dokumen</p>
		</div>
		<div class="row">
			<form class="form form-inline" name="srcForm" method="post" action="?">
				<input type="hidden" name="mod" value="<?php echo $mod;?>">
				<div class="form-group">
    				<label for="srcEms">Name</label>
    				<input type="text" name="srcEms" class="form-control">
  				</div>
  				<button name="btnCari" type="submit" class="btn btn-default"> Cari </button>
			</form>
		</div>
		<hr>
			<?php

			if(!empty($_POST['srcEms'])){
				$filt	= " AND ppkp_no_ems='$_POST[srcEms]'";
				$srcVal	= $_POST['srcEms'];
			} else{
				$filt	= "";
				$srcVal	= "";
			...
			$rowPerPage=5;
			$querycount	= $DBcon->query("SELECT * FROM ppkp_header a JOIN ppkplaststatus b ON a.ppkp_id=b.idlaststatus WHERE laststatus='5' $filt");
		...
		<div class="row">
				<form role="form" class="form-inline" name="pageForm" method="post" action="?">
					...
					<input type="hidden" name="srcEms" value="<?php echo"$srcVal";?>">
		...
		<div class="row">
			<form method="post" action="?">
				<input type="hidden" name="mod" value="33">
			...
			<table class="table table-condensed">
				...
				<?php
					$query	= $DBcon->query("SELECT * FROM ppkp_header a JOIN ppkplaststatus b ON a.ppkp_id=b.idlaststatus WHERE laststatus='5' $filt ORDER BY ppkp_id ASC LIMIT $startNum,$rowPerPage");
					$nom=$startNum+1;
