...
<div class="container">
	<div class="row">
		<div class="col-md-2"><p class="text-muted"><strong>Tgl Upload</strong></p></div>
		...
		<form class="form" method="post" action="?">
			...
			<div class="form-group col-md-4">
			    <select name="fltStat" class="form-control">
			    	...
			    	$qSlc1=$DBcon->query("SELECT * FROM npd_status WHERE npds_id='$fltStat'");
			    	$hSlc1=$qSlc1->fetch_array();
			    	?>
			    	<option value=<?php echo $hSlc1['npds_id']; ?>><?php if(!empty($fltStat)){ echo $hSlc1['npds_uraian']; } else { echo "--Semua--";}?></option>
			    	<option value="">--Semua--</option>
			    	<?php
			    	$qSlc2=$DBcon->query("SELECT * FROM npd_status WHERE npds_id!='$fltStat'");
			    	while($hSlc2=$qSlc2->fetch_array()){ ?>
			    		<option value=<?php echo $hSlc2['npds_id']; ?>><?php echo $hSlc2['npds_uraian']; ?></option>
			    	<?php
			    	}
			    	?>
			    </select>
	...
	<div class="row">
		...
		<?php
			if(isset($_POST['submit']) || isset($_POST['pageNum']) || isset($_POST['nextBut']) || isset($_POST['previousBut']) || isset($_POST['nextButLast']) || isset($_POST['previousButFirst']) ){
				...
				if(!empty($fltStat)){
					$filt.=" AND npd_flag_last_status='$fltStat'";
				}

				$changepjt=filtpjt($petugas,$DBcon);
				$filt.=str_replace("id_pjt", "npd_id_pjt", $changepjt);

				$sqlhal="SELECT * FROM npd_header WHERE DATE(npd_waktu_upload)>='$tgl_awal_sql' AND DATE(npd_waktu_upload)<='$tgl_akhir_sql' $filt";
				//echo $sqlhal;
				...
				$sql="SELECT * FROM npd_header WHERE DATE(npd_waktu_upload)>='$tgl_awal_sql' AND DATE(npd_waktu_upload)<='$tgl_akhir_sql' $filt";
				$sql.=" ORDER BY npd_waktu_upload ASC LIMIT $startNum,$rowPerPage";
				$query=$DBcon->query("$sql");
				$i=$startNum;
				while($data=$query->fetch_array()){
					$i++;
					$statakir= $data['npd_flag_last_status'];
					if($statakir==3){ $trstyle="class='success'";}
					elseif($statakir==2){ $trstyle="class='warning'";}
					elseif($statakir==4){ $trstyle="class='danger'";}
					else{$trstyle="";}
		?>	
					<tr <?php echo $trstyle;?>>
						<td><?php echo $i; ?></td>
						...
						<td><?php echo getprosesnpd($data['npd_flag_last_status'],$DBcon); ?></td>
						<td><?php echo getnamauser($data['npd_id_petugas'],$DBcon); ?></td>
						<td><button id="<?php echo $data['npd_id']; ?>" class="btn btn-default btn-show-detail"><span class="glyphicon glyphicon-zoom-in"></span>&nbsp;Detail</button></td>
					</tr>
		<?php
				}
			}
