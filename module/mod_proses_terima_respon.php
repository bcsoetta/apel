<?php
    ...
    foreach($_POST['flagproses'] as $value){        
        $exec_status=$DBcon->query("INSERT INTO tbl_batch_proses_respon (idbatch,idheader,waktu,idpetugas) VALUES ('$nobatch','$value','$tglinput','$petugas')");
        $exec_ctl=$DBcon->query("INSERT INTO pibk_ctl(header_seq_id, ctl_stat, ctl_time, user_id, flag_proses) VALUES ('$value', '4','$tglinput','$petugas','1')");
        $updatelaststat=$DBcon->query("UPDATE laststatus SET laststatus='4', wakturekam='$tglinput' WHERE idlaststatus='$value'");
        $i++;
        $count++;
