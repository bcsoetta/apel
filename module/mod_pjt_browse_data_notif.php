	...
	if(isset($_POST['exec'])=="true"){
		...
		if(!empty($_FILES['lampiran']['name'])){
			...
			if ($file_ext=="pdf") 	{
				...
				$insertdokap=$DBcon->query("INSERT INTO pibk_dokap_tambahan(idheader,namafile,waktuupload) VALUES('$respon_id_header','$got_name','$respon_waktu')");
				
				if($insertdokap){
					$insertdata=$DBcon->query("INSERT INTO pibk_ctl(header_seq_id, ctl_stat, ctl_time, user_id, flag_proses, uraian_proses) VALUES ('$respon_id_header', '10','$respon_waktu','$petugas','2','$respon_isi')");
					$updatedata=$DBcon->query("UPDATE laststatus SET laststatus='10',wakturekam='$respon_waktu' WHERE idlaststatus='$respon_id_header'");
	...
	<div class="row">
		...
		<?php
			if(isset($_POST['submit']) || isset($_POST['pageNum']) || isset($_POST['nextBut']) || isset($_POST['previousBut']) || isset($_POST['nextButLast']) || isset($_POST['previousButFirst']) ){
				...
				$sqlhal.=" AND t1.posisi='1' AND t2.laststatus='3' $filt";
				...
				$sql.=" AND t1.posisi='1' AND t2.laststatus='3' $filt";
		...
		?>	
					<tr>
						...
						<td><a class="btn btn-warning" href="?mod=37&id=<?php echo $data['idpibk_header']; ?>" target="_self">respon</a></td>
					</tr>
		<?php
				}
			}
